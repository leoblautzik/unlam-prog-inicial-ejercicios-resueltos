...
int main()
{
    ...
    if (participantes > 0)
    {
        ...
        if (t_promedio > 0)
        {
            printf("Hubo %d autos que superaron el tiempo promedio de %5.2f "
                   "seg.\n",
                   autos_por_encima_del_promedio(tiempos, t_promedio, TAM),
                   t_promedio);
        }
    }
...
int cargar_tiempos(float tiempos[], int tam)
{
    ...
    while (nro_auto > 0 && cant < tam)
    {
        if (tiempos[nro_auto - 1] != 0)
        {
            cant++;
        }
